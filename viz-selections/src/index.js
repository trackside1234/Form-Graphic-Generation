...
const SPEEDMAP_TEMPLATES = {
  8:  { master: "FF_Speedmap_8",  r1: Array(8).fill("10"),  state: "1" },
  12: { master: "FF_Speedmap_12", r1: Array(12).fill("65.0"), state: "1" },
  16: { master: "FF_Speedmap_16", r1: Array(16).fill("100"), state: "0" },
  24: { master: "FF_Speedmap_24", r1: Array.from({length:24}, (_, i) => (i === 0 || i === 23) ? "100" : "50"), state: "0" }
};
...
function speedmapRunnerXml(runner, r1) {
  return `<element name="${runner.elementName}"><entry name="data"><entry name="0101">${xmlEscape(runner.speedMap)}</entry><entry name="0101.R1">${xmlEscape(r1)}</entry><entry name="0102">${xmlEscape(String(runner.runnerNumber))}.</entry><entry name="0103">${xmlEscape(String(runner.horseName).toUpperCase())}</entry></entry></element>`;
}
function buildSpeedMapElement(env, race, runners, template, elementName) {
  const title = String(race.meeting_name || race.venue_name || "").toUpperCase();
  const raceNumber = Number(race.race_number || 0);
  const count = runners.length;
  const state = template.state;
  const nested = runners.map((r, i) => speedmapRunnerXml({ ...r, elementName: i === 0 ? "element" : `element#${i + 1}` }, template.r1[i])).join("");

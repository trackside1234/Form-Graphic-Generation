...
function meetingNumberOf(m) {
  const candidates = [m?.meeting_number, m?.meetingNumber, m?.meeting_no, m?.meeting_no_today, m?.sequence, m?.order, m?.meeting_order, m?.meeting_order_number];
...
function compactMeeting(m, country) {
  ...
  return {
    id: String(m?.id ?? m?.meeting_id ?? ""), meeting_id: m?.meeting_id ?? m?.id ?? null,
